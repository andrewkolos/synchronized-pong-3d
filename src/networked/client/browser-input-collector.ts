...
import { PaddleInput } from '../../networked/entities';
...
export class Pong3DBrowserInputCollectionStrategy implements InputCollectionStrategy {
  ...
  constructor(context: Pong3DBrowserInputCollectionStrategyContext) {
    this.mappings = context.keyMappings;
    this.game = context.game;
    this.targetEntityId = context.playerEntityId;
  ...
  public getInputs(dt: number): InputForEntity[] {
    const input: InputForEntity = {
      ...
      input: this.getPaddleMoveInput(dt)
    }

    return [input];
  }

  private getPaddleMoveInput(dt: number): PaddleInput {
  ...
  private getInputFromControls(dt: number): PaddleInput {
    ...
    if (this.isKeyDown(this.mappings.rotatePaddleLeft)) {
      input.dzRot = rotateSpeedPerMs * dt;
    }
